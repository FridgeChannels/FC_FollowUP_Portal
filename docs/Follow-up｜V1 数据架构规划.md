...
第一版采用四层执行数据库，并增加一张独立模板库和一张渠道容量配置表：
...
5. **Follow-up TemplateDB**：统一管理五个渠道使用的消息模板、电话话术和语音信箱话术。
6. **Follow-up ChannelCapacityDB**：维护五个渠道每个工作日允许排入的最大任务数量。

核心原则：

> ClientDB 和 KeyPersonDB 是主数据；Follow-up ClientDB 和 Follow-up ContactDB 定义本轮执行范围；Follow-up ChannelCapacityDB 提供每日渠道容量约束；Follow-up TaskDB 记录计划；Follow-up ConversationDB 管理消息内容和实际互动状态；Follow-up TemplateDB 只提供可选的标准内容来源。
> 
...
Follow-up ClientDB / Follow-up ContactDB
        ↓ 生成计划
Follow-up TemplateDB ── 可选模板 ──→ Follow-up TaskDB / Follow-up ConversationDB
...
管理当前进入跟进范围的客户，以及客户级 Owner、推进状态、Current CP 和最近互动。
...
| Owner | Person | 是 | 当前负责该客户的 Owner |
| Follow-up Status | Status | 是 | 客户整体跟进状态；选项见 4.3 |
| Current CP | Select | 是 | 记录最近已经完成的 CP，默认值为 NONE |
...
- **Unassigned**：客户已进入 Follow-up ClientDB，但尚未分配 Owner。
- **Not Started**：Owner 已确定，但尚未开始执行任何 Follow-up Task。
- **In Progress**：客户正在通过自动或人工方式推进；无论客户是否已经回复，都属于此状态。
- **Completed**：本轮客户跟进目标已经完成，不再需要继续生成标准跟进任务。
- **Terminated**：本轮跟进目标尚未完成，但已决定停止推进；具体原因写入中文 Notes。未来需要重新联系时可以恢复为 Not Started。

### 4.4 Current CP

Current CP 只记录该客户**最近已经完成的 CP**，默认值为 `NONE`。只有同时满足对应完成标准并具备可核验的 Evidence 后，才能更新。

| Option | 中文定义 | 完成标准 | Evidence |
| --- | --- | --- | --- |
| NONE | 尚未完成任何 CP。 | 没有任何一个 CP 达到完整完成标准。 | 无 |
| Post-Tap Brand Experience Delivered | 品牌定制的 Post-tap 体验已经完成，并已交付给 Connector 或 Owner 实际体验。 | Brand Customized Post-tap 已完成；Connector 或 Owner 已收到 FC 产品；对方可以实际 Tap 并访问该品牌体验。 | 体验链接、测试记录、交付对象、交付日期 |
| Sample Delivered to Owner | 正确 Owner 已经识别并收到 Sample，同时已具备后续直接推进所需的必要联系方式。 | 正确 Owner 已识别；Owner 已收到 Sample；Owner Fire Cover Complete。 | Owner 身份记录、联系人信息、引荐记录、签收或确认记录 |
| Owner Input Completed | Owner 的业务目标及必要输入已经收集完成，并形成可进入 Review 的客户专属 Plan。 | Business Objective 已确认；必要业务流程、事实和限制已记录；AI 已生成客户专属 Plan；FC 已完成人工审核；Plan 已达到可进入 Review 的完整度。 | Guided Input 记录、Plan 版本、FC Review 记录 |
| Plan Review Completed | Owner 或相关决策人已完成 Plan Review，关键反馈和未决事项已记录，当前 Plan 已确认或形成明确修改项。 | Owner 或决策相关人实际参会；双方讨论 Business Objective、Scope、Customer Experience、Measurement、Waste Control；反馈和未决问题已记录；Plan 已确认或修改项已明确；Plan 已交付给 Owner。 | Meeting Notes、反馈记录、Plan 更新版本、交付记录 |
| Paid / Deal Confirmed | 交易已经通过实际付款或财务确认正式成立。 | 付款已到账，或由财务确认付款成功。 | 付款记录、财务确认、Invoice 状态 |
| Fulfillment Delivered | 从产品设计、生产、技术配置到交付和上线准备均已完成，FC 侧不存在未解决的交付阻塞。 | Physical Product Design 完成；客户批准最终设计；Production 完成；Technical Setup 完成并测试通过；Distribution Preparation 完成；产品已交付；已经 Live 或不存在 FC-side blocker。 | Design approval、production record、technical QA、delivery record、go-live record |
| Scale | 首轮交付已经完成测量与复盘，新增 Scope 和对应付款均已确认。 | Measurement 和复盘已完成；新增 Scope 已确认；扩大 Scope 的付款已完成。 | Measurement report、Scale recommendation、Scope confirmation、付款记录 |
...
- 转为人工处理时，只将 Follow-up Contact 的 Follow-up Mode 更新为 Manual，不改变 Follow-up Status。

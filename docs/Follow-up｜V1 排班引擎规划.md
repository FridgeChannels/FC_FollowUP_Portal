...
- 任务只安排在周一至周五的美国工作时间（`America/New_York`，默认 09:00–17:00），并跳过美国联邦假日。
- 目标工作日容量不足或日内分钟槽放不下时，任务自动顺延到下一个可用美东工作日，直到找到可用容量。
- `Scheduled At` 记录计划发送时刻（精确到分钟的 datetime）；历史仅到天的值按当日 09:00 ET 参与间隔计算。
- 同渠道任务在同一天内按 ChannelCapacityDB 的 `Time interval`（分钟）错开；缺失或非法时回退为 5 分钟。
- 当天排班从「当前美东时间」向后对齐到下一可用分钟；未来工作日从 09:00 ET 起排。
- 每条 Follow-up Task 只占用所属 Channel 当日的一个容量名额。
- 本地测试可用环境变量 `SCHEDULE_TEST_MODE=1`：从当前时间起每 5 分钟排槽，全部落在当天，并忽略工作窗口、联邦假日、Daily Max 与客户每日一渠限制。
...
3. 读取每个渠道当前的 Daily Max 与 Time interval。
4. 统计目标日期该渠道的总占用数量，以及已占用的发送时刻。
5. 将任务排入最早有剩余渠道容量、且满足美国工作窗口与间隔的分钟槽。
6. 在正式创建前重新检查渠道总容量与时刻占用，避免并发操作造成超额。
...
按照优先级排序
    ↓
从 Preferred Start Date 开始查找美东工作日（跳过联邦假日）
    ↓
读取 Channel Daily Max 与 Time interval
    ↓
统计 Channel Allocated 与已占用发送时刻
    ↓
计算 Channel Available
    ↓
检查该客户当日是否已经安排渠道任务
    ↓
渠道容量可用且客户当日尚无渠道任务：在 09:00–17:00 ET 内按间隔分配分钟槽（当天从当前美东时间对齐）
渠道容量不足、窗口放不下或客户当日已有渠道任务：自动尝试下一个美东工作日
    ↓
生成预览结果，包括各时刻、渠道、Owner 和同一客户的渠道分布
    ↓
用户确认
    ↓
重新校验 Channel 容量与时刻占用
    ↓
创建 Task（Scheduled At = datetime）
```

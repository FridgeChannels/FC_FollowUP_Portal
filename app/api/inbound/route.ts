...
import { notifyInboundReceived } from "@/lib/notify";
import { retrievePage } from "@/lib/notion/client";
import { mapFollowupClientPage } from "@/lib/notion/followup-clients";
import { ingestInboundCold } from "@/lib/notion/inbound-cold";
import { InboundReplyError } from "@/lib/notion/inbound-errors";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await ingestInboundCold(body, async (target) => {
    ...
    return Response.json(publicResult, { status: 201 });
  } catch (error) {

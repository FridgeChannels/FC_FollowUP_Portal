...
export default function OutreachWorkspace() {
  ...
  useEffect(() => {
    ...
    if (pathname.startsWith("/inbox") || pathname.startsWith("/call-tasks")) {
      router.replace(state.currentRole === "Human Responder" ? "/customers" : pathname.replace(/^\/(inbox|call-tasks)/, "/tasks"));
      return;
    }
    if (state.currentRole === "Human Responder" && screen === "ReplyTask") {

...
export function InteractionFeed({ interactions, contacts, customerId: customerIdProp, maxHeight = "max-h-[520px]" }: { interactions: Interaction[]; contacts: Contact[]; customerId?: string; maxHeight?: string }) {
  ...
  return <div>
    <div className="border-b bg-white px-5 py-4">
      <div className="flex items-center">{cpCodes.map((cp,index)=>{const current=cp===currentCp;const completed=index<currentIndex;const selected=cp===selectedCp;const selectable=index<=currentIndex;const reached=index<currentIndex;return <Fragment key={cp}><button type="button" disabled={!selectable} aria-pressed={selected} onClick={()=>selectable&&setSelectedCp(cp)} className={`min-w-0 flex-1 rounded-xl border px-3 py-2 text-left transition ${selected?"border-violet-500 bg-violet-50 ring-2 ring-violet-100":selectable?"border-slate-200 bg-slate-50 hover:border-violet-300 hover:bg-violet-50/50":"cursor-not-allowed border-slate-200 bg-slate-50 opacity-65"}`}><div className={`text-xs font-bold ${selected?"text-violet-700":completed?"text-emerald-700":"text-slate-400"}`}>{cp}</div><div className="mt-0.5 truncate text-[10px] text-slate-500">{current?"Current":completed?"Completed":"Upcoming"}</div></button>{index<cpCodes.length-1&&<span className="grid w-6 shrink-0 place-items-center" aria-hidden><ChevronRight className={`size-4 ${reached?"text-emerald-500":current?"text-violet-400":"text-slate-300"}`}/></span>}</Fragment>})}</div>
    </div>
    ...
    {selectedActivity.length>0&&<ActivityRows visible={selectedActivity} contacts={contacts} maxHeight={maxHeight}/>}
  </div>;
}
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2"><h4 className="text-sm font-bold text-slate-950">{interaction.channel || interaction.type}</h4>{isReply && <Badge variant="outline" className="text-[10px]">Inbound</Badge>}{isReply && <Badge className="bg-rose-600 text-[10px] text-white hover:bg-rose-600">This is a reply</Badge>}{interaction.direction && !isReply && <Badge variant="outline" className="text-[10px]">{interaction.direction}</Badge>}{interaction.outcome && <Badge variant="secondary" className="text-[10px]">{interaction.outcome}</Badge>}</div>
            ...
            {isReply && <BrandReplyBox customerId={interaction.customerId} interaction={interaction}/>}

...
import {
  addDays, Channel, Contact, CPCode, createSeedState, dateOnly, roleCapabilities,
  normalizeRole, Role, uid, WorkspaceState, BombTemplate, CPStage,
} from "@/lib/outreach-domain";
import { buildTemplateVariableContext, ownerNameFromContacts, resolveOutboundFields } from "@/lib/template-variables";
...
type Store = {
  ...
  cancelBomb: (customerId: string, instanceId: string) => Result;
...
function stopBomb(draft: WorkspaceState, customerId: string, reason: string) {
  ...
  draft.interactions.unshift({id:uid("int"),customerId,bombInstanceId:instance.id,cp:customer.cp,type:"Bomb",title:"OmniReach stopped",content:`${reason} · Future actions cancelled${released?` · ${released} phone slot released`:""}`,createdAt:draft.simulatedDate});
  return released;
}
...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const setRole=(role:Role)=>mutate(d=>{d.currentRole=role;d.currentUserId=role==="Caller"?"u_alex":role==="AccountManager"?"u_mike":"u_sarah";});
  ...
  const launchBomb=(customerId:string,bombId:string,targetContactId?:string,stepCopies?:Record<string,{subject?:string;content?:string;callGoal?:string;script?:string}>):Result=>{
    ...
    mutate(d=>{const customer=d.customers.find(c=>c.id===customerId);const bomb=d.bombs.find(b=>b.id===bombId);if(!customer||!bomb){result={ok:false,message:"Brand or OmniReach not found"};return;}if(customer.activeBombId){result={ok:false,message:"This Brand already has an active OmniReach"};return;}if(bomb.status!=="Active"){result={ok:false,message:"Only Active OmniReach can be launched"};return;}if(!targetContactId){result={ok:false,message:`Select a ${bomb.targetRole} contact before launching`};return;}const target=customer.contacts.find(c=>c.id===targetContactId);if(!target||target.role!==bomb.targetRole){result={ok:false,message:`Select a valid ${bomb.targetRole} contact before launching`};return;}
      ...
      bomb.steps.forEach((step,index)=>{const planned=addDays(d.simulatedDate,index);let actual=planned;let status:"Scheduled"|"Sent"|"Skipped"="Scheduled";let note:string|undefined;if(!channelReachable(target,step.channel)){status="Skipped";note=`${step.channel} unavailable for ${target.name}`;}
        const actionId=uid("act");let callTaskId:string|undefined;
        const copy=stepCopies?.[step.id];const vars=buildTemplateVariableContext({companyName:customer.name,hasContact:true,contactName:target.name,contactTitle:target.title,contactRole:target.contactRole,email:target.email,phone:target.phone,ownerName:ownerNameFromContacts(customer.contacts),ownerOrConnector:target.role,linkedinUrl:target.linkedin});const resolved=resolveOutboundFields({channel:step.channel,subject:copy?.subject??step.subject,content:copy?.content??step.content,callGoal:copy?.callGoal??step.callGoal,script:copy?.script??step.script},vars);const content=step.channel==="Email"&&resolved.subject?`Subject: ${resolved.subject}\n\n${resolved.content}`:resolved.content;
      ...
      customer.activeBombId=instanceId;customer.status="Bomb Running";customer.updatedAt=d.simulatedDate;bomb.launches++;d.interactions.unshift({id:uid("int"),customerId,contactId:target.id,bombInstanceId:instanceId,cp:bomb.cp,type:"Bomb",title:"OmniReach started",content:`${bomb.name} · Version ${bomb.version}`,createdAt:d.simulatedDate});addAudit(d,"OmniReach launched",customerId,undefined,`${bomb.name} V${bomb.version}`);result={ok:true,message:`${bomb.name} launched`,id:instanceId};
    });return result;
  };

  const cancelBomb=(customerId:string,instanceId:string):Result=>{let result:Result={ok:false,message:"OmniReach not found"};mutate(d=>{const customer=d.customers.find(item=>item.id===customerId);const instance=d.bombInstances.find(item=>item.id===instanceId);if(!customer||!instance||instance.customerId!==customerId)return;if(instance.status!=="Running"&&instance.status!=="Paused"){result={ok:false,message:"This OmniReach is no longer running"};return;}if(customer.activeBombId!==instanceId){result={ok:false,message:"This OmniReach is no longer active"};return;}const released=stopBomb(d,customerId,"Stopped manually");customer.status="Waiting for Reply";customer.updatedAt=d.simulatedDate;addAudit(d,"OmniReach stopped manually",customerId,instance.templateName,`Cancelled ${released} future phone task${released===1?"":"s"}`);result={ok:true,message:"OmniReach stopped; remaining tasks cancelled"};});return result;};
  ...
  const saveBomb=(bomb:BombTemplate):Result=>{mutate(d=>{const index=d.bombs.findIndex(b=>b.id===bomb.id);d.bombs[index]={...bomb,version:bomb.status==="Active"?bomb.version+1:bomb.version,updatedAt:d.simulatedDate};addAudit(d,"OmniReach template edited",undefined,`V${bomb.version}`,`V${d.bombs[index].version}`);});return{ok:true,message:"OmniReach version saved"};};
  const createBomb=(bomb:Omit<BombTemplate,"id"|"version"|"launches"|"updatedAt">):Result=>{let id="";mutate(d=>{id=uid("b");d.bombs.unshift({...bomb,id,version:1,launches:0,updatedAt:d.simulatedDate});addAudit(d,"OmniReach template created",undefined,undefined,bomb.name);});return{ok:true,message:"OmniReach created",id};};
  const setBombStatus=(bombId:string,status:BombTemplate["status"]):Result=>{mutate(d=>{const b=d.bombs.find(b=>b.id===bombId);if(b){const prev=b.status;b.status=status;b.updatedAt=d.simulatedDate;addAudit(d,"OmniReach status changed",undefined,prev,status);}});return{ok:true,message:`OmniReach ${status.toLowerCase()}`};};
  ...
  const value:Store={state,hydrated,can,reset,setRole,advanceDay,launchBomb,cancelBomb,receiveReply,sendHumanReply,changeCP,createFollowUp,resolveInbox,pauseBrand,assignBrand,createCallTask,createBrand,importBrands,updateContact,closeBrand,reassignCall,updateCallerCapacity,saveBomb,createBomb,setBombStatus,updateCPStage,updateIntegration,fillTodayCapacity,simulateFailure};

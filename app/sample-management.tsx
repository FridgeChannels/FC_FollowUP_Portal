...
import { Check, Clock3, Copy, ExternalLink, Link2, MousePointerClick, UserRound } from "lucide-react";
        ...
        <section className="mt-10" aria-label="Click activity">
          ...
          <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200 bg-white">
            <table className="min-w-[820px] w-full text-left">
              ...
              <tbody>
                {clicks.map((click) => <tr key={click.id} className="border-b border-slate-100 last:border-0">
                  <td className="whitespace-nowrap px-3 py-4 text-sm font-semibold">{click.person}</td>
                  ...
                  <td className="whitespace-nowrap px-3 py-4 text-xs text-slate-500"><time dateTime={click.clickedAt}>{formatDate(click.clickedAt)}</time></td>
                </tr>)}
              </tbody>
            </table>
          </div>
...
function AmazonProductEditor({ customerId, initialProduct, onSaved }: { customerId: string; initialProduct: AmazonSampleProduct; onSaved: (product: AmazonSampleProduct) => void }) {
  ...
  return <section className="mt-6 rounded-xl border border-slate-200 bg-white p-4 sm:p-5" aria-label="Amazon product">
    <div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold">Amazon product</h2><p className="mt-0.5 text-xs text-slate-500">Add the details used to complete this sample link.</p></div><Button size="sm" className="min-h-10 shrink-0" disabled={!dirty || saving || uploading} onClick={() => void save()}>{saving ? "Saving…" : "Save"}</Button></div>
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <label className="grid gap-1 text-sm font-medium">Product name<input value={product.name} onChange={(event) => update("name", event.target.value)} className="h-10 rounded-lg bg-slate-50 px-3 text-sm font-normal outline-none ring-1 ring-slate-200 focus:bg-white focus:ring-2 focus:ring-violet-500" /></label>
      <label className="grid gap-1 text-sm font-medium">Price <span className="font-normal text-slate-500">(optional)</span><input value={product.price} onChange={(event) => update("price", event.target.value)} placeholder="$29.99" className="h-10 rounded-lg bg-slate-50 px-3 text-sm font-normal outline-none ring-1 ring-slate-200 focus:bg-white focus:ring-2 focus:ring-violet-500" /></label>
      <label className="grid gap-1 text-sm font-medium">Amazon product link<input type="url" value={product.url} onChange={(event) => update("url", event.target.value)} placeholder="https://www.amazon.com/dp/..." className="h-10 rounded-lg bg-slate-50 px-3 text-sm font-normal outline-none ring-1 ring-slate-200 focus:bg-white focus:ring-2 focus:ring-violet-500" /></label>
      <div className="grid gap-1"><label className="text-sm font-medium" htmlFor="product-image-url">Product image URL</label><input id="product-image-url" type="url" value={product.imageUrl} onChange={(event) => update("imageUrl", event.target.value)} className="h-10 rounded-lg bg-slate-50 px-3 text-sm outline-none ring-1 ring-slate-200 focus:bg-white focus:ring-2 focus:ring-violet-500" /><label className="inline-flex min-h-8 w-fit cursor-pointer items-center text-xs font-medium text-violet-700 hover:text-violet-900">{uploading ? "Uploading…" : "Upload product image"}<input type="file" accept="image/*" className="sr-only" disabled={uploading} onChange={(event) => void upload(event.target.files?.[0])} /></label></div>
    </div>
...
function SummaryStat({ label, value, icon: Icon, compact }: { label: string; value: string; icon: typeof Clock3; compact?: boolean }) {
  return <div className="min-w-0 rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-center gap-1 text-[11px] text-slate-500"><Icon className="size-3.5 shrink-0 text-violet-600" /><span className="truncate">{label}</span></div><p className={`mt-2 break-words font-semibold tracking-tight ${compact ? "text-xs leading-5 sm:text-sm" : "text-2xl"}`}>{value}</p></div>;
}
